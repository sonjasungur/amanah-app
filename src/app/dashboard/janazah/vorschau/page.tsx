  ...
  return (
    <div data-testid="janazah-preview">
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-page-title font-bold text-foreground mb-2">Vorschau deiner Janazah-Wünsche</h1>
          <p className="text-body text-muted max-w-2xl leading-relaxed">
            Prüfe deine Angaben vor dem Abschluss — alle Felder können jederzeit bearbeitet werden.
          </p>
        </div>
      </div>

      <aside className="mb-6 rounded-xl border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted leading-relaxed">
        Diese Angaben spiegeln deine persönlichen Wünsche wider. Sie haben keine rechtliche oder religiöse Verbindlichkeit.
      </aside>
      ...
      <div className="space-y-6">
        {visibleSections.map((section, sectionIndex) => {
          ...
          const missingCritical = section.fields.filter((field) => {
            return CRITICAL_FIELDS.has(field.key) && formatValue(getNestedValue(values, field.key)) === null;
          });
            ...
            >
              ...
              {missingCritical.length > 0 && (
                <div className="mb-4 flex items-start gap-2 rounded-lg border border-warning/30 bg-warning/5 px-3 py-2 text-sm text-foreground">
                  ...
                  <div>
                    <p className="font-medium text-warning mb-1">Noch offen</p>
              ...
              {filledFields.length === 0 && missingCritical.length === 0 ? (
                <p className="text-sm text-muted italic">Noch keine Angaben für diesen Bereich.</p>
              ) : (
